...
public class PythonModelServiceClient {
    ...
    public PythonModelResponse predictRanking(PythonModelRequest request) {
        try {
            log.debug("Calling Python model service for user: {}", request.getUserAcademic().getUserId());
            ...
            PythonModelResponse response = webClient.post()
                ...
                .onErrorResume(e -> {
                    log.error("Python model service error: {}", e.getMessage());
                    return Mono.just(createFallbackResponse());
                })
                .block();

            if (response != null && response.getRankedPosts() != null) {
                log.debug("Received {} ranked posts from Python service", response.getRankedPosts().size());
                return response;
            }

            log.warn("Received empty response from Python service, using fallback");
